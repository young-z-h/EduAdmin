...
public class StudentController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String actorId = request.getParameter("teacherId");
        String user_str = JSON.toJSONString(request,SerializerFeature.IgnoreNonFieldGetter);
        //创建JSON对象message，以便往前端响应信息
        JSONObject message = new JSONObject();
        try {
            //如果id = null, 表示响应所有教师对象，否则响应id指定的教师对象
            if (id_str == null && actorId ==null) {
                responseStudents(response);
            }
            else if(actorId!=null){
                int user_id = Integer.parseInt(actorId);
                User user = UserService.getInstance().find(user_id);
                int teacher_id = user.getActor().getId();
                System.out.println(teacher_id);
                Teacher teacher = TeacherService.getInstance().find(teacher_id);
//                System.out.println(teacher);//
                int department_id = teacher.getDepartment().getId();
                Collection<Student> studentsByDepartment = StudentService.getInstance().findByDepartment(department_id);
                String teacher_json = JSONObject.toJSONString(studentsByDepartment,SerializerFeature.IgnoreNonFieldGetter);
