...
public class StudentDao {
    ...
    public Collection<Student> findByDepartment(Integer id) throws SQLException {
        Connection connection = JdbcHelper.getConn();
        Collection<Student> students = new TreeSet<Student>();
        String find_str = "SELECT * FROM student,actor where student.id=actor.id and student.department_id=?";
        PreparedStatement preparedStatement = connection.prepareStatement(find_str);
    ...
    public Student findMe(User user) throws SQLException {
        Connection connection = JdbcHelper.getConn();
        String findMe_sql = "SELECT * from student,actor where student.id=actor.id and student.id=?";
        int id = user.getActor().getId();
        PreparedStatement preparedStatement = connection.prepareStatement(findMe_sql);
        preparedStatement.setInt(1, id);
        ResultSet resultSet = preparedStatement.executeQuery();
        Student student = null;
        if (resultSet.next()) {
