...
public class ExamDao {
    ...
    public boolean add(Exam exam) throws SQLException {
        Connection connection = JdbcHelper.getConn();
        String addProject_sql = "INSERT INTO exam(student_id,teaching_id) VALUES" + " (?,?)";
        //在该连接上创建预编译语句对象
        PreparedStatement preparedStatement = connection.prepareStatement(addProject_sql);
        //为预编译参数赋值
        preparedStatement.setInt(1, exam.getStudent().getId());
        preparedStatement.setInt(2, exam.getTeaching().getId());
